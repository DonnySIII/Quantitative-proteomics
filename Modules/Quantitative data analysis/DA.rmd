...
kable(data.frame(c("R-Analysis Version", "File", "Number of rows", "Origin File", "Date"),c(2.11,params$quant_df,nrow(data),params$origin_file,format(Sys.time(), '%y-%m-%d %H:%M:%S'))),format="html",col.names = NULL,booktabs = T, caption = "Analysis") %>%
...
if (normalization){
  ...
  # Plots
  df_raw_intensity = reshape2::melt(raw_abundance[-1])
  ...
  df_norm_intensity = reshape2::melt(normalized_abundance[-1])
...
na_distribution = data.frame(median=numeric(),sum_na=numeric())
# medians = c()
# sums_na = c()
...
# Keep ID column, log transform the rest
imputed_abundance_log = data.frame(
  Id = imputed_abundance[,1],
  log10(imputed_abundance[,2:ncol(imputed_abundance)])
)
colnames(imputed_abundance_log)[2:ncol(imputed_abundance_log)] = paste0(colnames(imputed_abundance)[2:ncol(imputed_abundance)],"_log")
...
imputationDF = reshape2::melt(imputed_abundance_log[,-1])
...
if(no_imputation){
  ...
  abundanceDF = reshape2::melt(abundance_log[,-1])
  colnames(abundanceDF) = c("Sample","log_abundance")
}

## imputed?
imputed = reshape2::melt(is.na(mean_abundance_repbio[,-1]))
...
```{r global_filtering}
##Parsing of data input file to separate by specific peptides##
...
df_median_deviation_before_processing = reshape2::melt(df_median_deviation_before_processing[,-1])
...
df_median_deviation_after_processing = reshape2::melt(df_median_deviation_after_processing[,-1])
...
df_cv_before_processing = reshape2::melt(df_cv_before_processing)
...
df_cv_after_processing2 = reshape2::melt(df_cv_after_processing)
...
if(parameters["Test.type",]%in%c("anova","kruskall")){
  ...
  reshape_abundance = reshape2::melt(intensity_df,id="Id")
...
for (comparison in comparisons_to_do){
  ...
  if(comparison%in%comparisons_to_display){
    ...
    if(length(current_statDF$significant[current_statDF$significant==T])>1){
      ...
      DT::datatable(significant_proteins_table,
      rownames = FALSE,
      extensions = 'Buttons',
      options = list(
        dom = 'Bfrtip',
      ...
      DT::formatStyle(
        columns = colnames(significant_proteins_table),
        fontSize = '8pt',
        rowHeight = '30px'
      )
  ...
  if (no_imputation) {
    to_add <- filtered_data[
      grepl(paste0("((", A_name, "|", B_name, ").*_noimpute_log$|Accession$|Label$)"), names(filtered_data))
    ][, -2]
  } else {
    to_add <- filtered_data[
      grepl(paste0("((", A_name, "|", B_name, ").*_imputed_log$|Accession$|Label$)"), names(filtered_data))
    ][, -2]
  }
  
  
  # keep only columns not already in output_t, except Accession which is the key
  non_key_cols <- setdiff(names(to_add), "Accession")

  names(to_add)[names(to_add) %in% non_key_cols] <- paste0(non_key_cols, "_", comparison)
  
  to_add <- to_add[, c("Accession", setdiff(names(to_add), c("Accession", names(output_t)))), drop = FALSE]
  
  output_t <- merge(output_t, to_add, by = "Accession", all = TRUE)
  
  # same idea for current_statDF
  stat_to_add <- current_statDF[, c("Accession", setdiff(names(current_statDF), c("Accession", names(output_t)))), drop = FALSE]
  
  output_t <- merge(output_t, stat_to_add, by = "Accession", all = TRUE)
...
```{r preparing_metadata, include=FALSE}

analysis_metadata <- data.frame(
  Field = c("R-Analysis Version", "File", "Number of rows", "Origin File", "Date"),
  Value = c(
    2.11,
    params$quant_df,
    nrow(data),
...
if(params$proline_source_file == "none"){
  ...
  source_file <- params$proline_source_file
  ext <- tolower(tools::file_ext(source_file))
  
  if (ext %in% c("xlsx", "xlsm")) {
    existing_file <- openxlsx::loadWorkbook(source_file)
  } else if (ext %in% c("txt", "tsv")) {
    
    # 1. Read the tab-separated file
    df <- read.delim(source_file, sep = "\t", header = TRUE, stringsAsFactors = FALSE)
    
    # 2. Create a new workbook
    existing_file <- openxlsx::createWorkbook()
    
    # 3. Add a worksheet (name can be anything meaningful)
    openxlsx::addWorksheet(existing_file, "Parsed_proteins_set")
    
    # 4. Write the data into it
    openxlsx::writeData(existing_file, "Parsed_proteins_set", df)
  ...
  addWorksheet(existing_file, "R_Metadata")
  
  writeData(existing_file, "R_Metadata", analysis_metadata, colNames = FALSE)
  
  metadata_style <- createStyle(fgFill = "lightblue")
  addStyle(
    existing_file,
    "R_Metadata",
    metadata_style,
    cols = 1:2,
    rows = 1:nrow(analysis_metadata),
    gridExpand = TRUE
  )
  
  setColWidths(existing_file, "R_Metadata", cols = 1:2, widths = 35)
