...
if (normalization){
  ...
  cat("Before merge: nrow(data) =", nrow(data), "\n")
  cat("Before merge: nrow(normalized_abundance) =", nrow(normalized_abundance), "\n")
  
  cat("Id class (data):", class(data$Id), "\n")
  cat("Id class (norm):", class(normalized_abundance$Id), "\n")
  
  cat("Unique Ids (data):", length(unique(data$Id)), "\n")
  cat("Unique Ids (norm):", length(unique(normalized_abundance$Id)), "\n")
  
  overlap <- intersect(as.character(data$Id), as.character(normalized_abundance$Id))
  cat("Overlap Ids:", length(overlap), "\n")
  
  if (length(overlap) == 0) {
    cat("Example Ids in data:\n"); print(head(as.character(data$Id), 10))
    cat("Example Ids in normalized_abundance:\n"); print(head(as.character(normalized_abundance$Id), 10))
  }
